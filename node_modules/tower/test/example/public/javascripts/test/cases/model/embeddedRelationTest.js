...
describeWith = function(store) {
  var post;
  post = null;
  return describe("Tower.Model.Relation.HasMany (Tower.Store." + store.name + ")", function() {
    beforeEach(function(done) {
      var _this = this;
      App.Post.store(store);
      App.Comment.store(store);
      return App.Post.destroy(function() {
        return App.Comment.destroy(function() {
    ...
    if (store !== Tower.Store.Memory) {
      return describe('embedded', function() {
        return describe('.create', function() {
          return test('existingPost.embeddedComments().create()', function(done) {
            var _this = this;
            return post.embeddedComments().create(function(error, comment) {
              assert.equal(comment.constructor.name, "Comment");
              return App.Post.store().findOne({
                id: 10
              }, {
                raw: true
              }, function(error, doc) {
                assert.equal(doc.embeddedComments.length, 1);
                return App.Comment.count(function(error, count) {
